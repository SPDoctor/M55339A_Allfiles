...
param(
    ...
    [string]$RootPath = "C:\M55339A_Allfiles",
...
foreach ($projectFile in $projectFiles) {
    ...
    try {
        ...
        if ($content -match 'Microsoft\.EntityFrameworkCore.*Version="8\.') {
            $content = $content -replace '(Microsoft\.EntityFrameworkCore[^"]*" Version=")8\.[0-9\.]+', '$1010.0.0'
...
if (-not $WhatIf -and $updatedCount -gt 0) {
    ...
    Write-Host "1. Run 'dotnet restore' in your solution directories" -ForegroundColor White
    Write-Host "2. Build your solutions to verify the upgrade" -ForegroundColor White
    Write-Host "3. Test your applications thoroughly" -ForegroundColor White
}
