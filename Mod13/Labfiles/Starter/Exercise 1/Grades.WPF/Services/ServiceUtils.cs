...
namespace Grades.WPF.Services
{
    class ServiceUtils
    {
        ...
        public async Task GetStudentsByTeacher(string teacherName, Action<IEnumerable<DataModel.Student>> callback)
        {
            if (!IsConnected())
                return;

            // Fetch students asynchronously by using the GradesService service
