...
namespace School
{
    class WordWrapper : IDisposable
    {
        ...
        /// </summary>
        private Microsoft.Office.Interop.Word.Range GetEndOfDocument()
        {
        ...
        {
            // Get the public key from the X509 certificate.
            var provider = this._certificate.GetRSAPublicKey();

            // Create an instance of the Aes algorithm which we will use to encrypt the data with.
        ...
        void ValidateX509Config()
        {
            ...
            if (this._certificate == null)
                throw new NotSupportedException(
                    string.Format("If you are using asymmetric encryption mode you must ensure there is a certificate in your store with the subject name '{0}'.", this._certificateSubjectName));
        ...
        public void CombineEncryptedReports(string directoryPath, string combinedReportPath)
        {
            ...
            // Get all encrypted report XML files.
            var reportsToProcess = Directory.GetFiles(directoryPath, "*.encrypted");
            ...
            // Delete decrypted reports.
            foreach (var report in reportsToProcess)
            {
                var outputPath = report.Replace(".encrypted", ".xml");
        ...
        public void DecryptReport(string reportPath)
        {
            ...
            var outputPath = reportPath.Replace(".encrypted", ".xml");
            File.WriteAllBytes(outputPath, decryptedReport);
        }
        ...
        {
            // Get the private key from the X509 certificate.
            var provider = this._certificate.GetRSAPrivateKey();
            ...
            using (var algorithm = Aes.Create())
            {
                using (var inStream = new MemoryStream(bytesToDecrypt))
                {
                    ...
                    // Read the key and IV lengths starting from index 0 in the in stream.
                    ...
                    // Determine the starting position and length of the data.
                    ...
                    // Create the byte arrays for the encrypted key, the IV, and the encrypted data.
                    var encryptionKey = new byte[convertedKeyLength];
                    var iv = new byte[convertedIvLength];
                    var encryptedData = new byte[dataLength];

                    // Read the key, IV, and encrypted data from the input stream.
                    ...
                    var decryptedKey = provider.Decrypt(encryptionKey, RSAEncryptionPadding.Pkcs1);
                    ...
                    using (var outStream = new MemoryStream())
                    {
                        using (var decryptor = algorithm.CreateDecryptor(decryptedKey, iv))
                        {
                            using (var decrypt = new CryptoStream(outStream, decryptor, CryptoStreamMode.Write))
                            {
                                ...
                                // Return the decrypted buffered data as a byte[].
                                return outStream.ToArray();
                            }
                        }
                    }
        ...
        public void Print(string filePath)
        {
            ...
            // Close the document without saving it to disk (to avoid storing the full report data in an unencrypted state).
            object oFalse = false;
